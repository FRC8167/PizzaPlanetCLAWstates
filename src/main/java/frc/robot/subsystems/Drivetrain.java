...
public void arcadeDrive(double throttle, double rotation) {
    ...
    // }
    //********************************************************************** */
   // SmartDashboard.putNumber("maxOutput Value", drivetrain.kDefaultMaxOutput);
    double speedmodifier = Constants.DRIVETRAIN_LIMIT_NOMINAL;

    if (RobotContainer.driverController.rightTrigger().getAsBoolean()) {
      speedmodifier = Constants.DRIVETRAIN_LIMIT_SNAIL;
    } else if (RobotContainer.driverController.leftTrigger().getAsBoolean()){
      speedmodifier = Constants.DRIVETRAIN_LIMIT_FAST;
    } else{
      speedmodifier = Constants.DRIVETRAIN_LIMIT_NOMINAL;
    }

    SmartDashboard.putNumber("Drive Output", speedmodifier * throttle * 100);
    drivetrain.arcadeDrive(drivetrain_flip * speedmodifier * -throttle, speedmodifier * rotation);

    //drivetrain.arcadeDrive(snail * drivetrian_flip * -throttle, snail * rotation, true);
    //drivetrain.arcadeDrive(drivetrian_flip * -throttle, rotation, true);
  }
...
public void periodic() {
  ...
  // SmartDashboard.putNumber("LFWD Current", RobotContainer._pdm.getCurrent(3));
  // SmartDashboard.putNumber("LAFT Current", RobotContainer._pdm.getCurrent(12));
  // SmartDashboard.putNumber("RFWD Current", RobotContainer._pdm.getCurrent(2));
  // SmartDashboard.putNumber("RAFT Current", RobotContainer._pdm.getCurrent(13));
  //SmartDashboard.putNumber("System Current", RobotContainer._pdm.getTotalCurrent());
  // SmartDashboard.putNumber("dd", RobotContainer._pdm.getTemperature());
}
...
/*************** Experimental Speed Limiter **************/
public Command setDriveTrainOutputLimit(double maxPercentOutput) {
  //SmartDashboard.putNumber("Drivetrain Max Output", drivetrain.);
  return Commands.runOnce(() -> drivetrain.setMaxOutput(maxPercentOutput));
}  
}
