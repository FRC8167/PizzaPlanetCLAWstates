...
public class Pivot_MM extends SubsystemBase {
	...
	public Pivot_MM() {
		...
		angleSensor = new AnalogPotentiometer(anglePotentiometer, 300, 0);
		...
		_talon.configMotionAcceleration(10000, Constants.kTimeoutMs);
	...
	public void periodic() {
		...
		SmartDashboard.putNumber("Manual Setpt", setpt);
		SmartDashboard.putNumber("Pivot AFF", arbFeedFwd);

		SmartDashboard.putNumber("Pot [counts]", anglePotentiometer.getValue());
		SmartDashboard.putNumber("AnglePot [deg]", angleSensor.get());
		
		SmartDashboard.putNumber("Pivot Motor Current", RobotContainer._pdm.getCurrent(14));
	...
	public void my_motionMagic_Jog(double deg){
		//Need to add clamp input code to prevent over travel
		m_targetPos = deg_To_Raw_Sensor_Counts(deg);
		//_talon.set(TalonFXControlMode.MotionMagic, m_targetPos);
		_talon.set(TalonFXControlMode.MotionMagic, m_targetPos, DemandType.ArbitraryFeedForward, arbFeedFwd);
	}

	public void slow_MM_params(double deg){
		_talon.configMotionCruiseVelocity(4000, Constants.kTimeoutMs);
		_talon.configMotionAcceleration(2000, Constants.kTimeoutMs);
	} 

	public void restore_MM_params(double deg){
		_talon.configMotionCruiseVelocity(11000, Constants.kTimeoutMs);
	...
	public void my_motionMagic_Run(double deg) {
		...
		arbFeedFwd = getFeedForward(0.15);
		_talon.set(TalonFXControlMode.MotionMagic, targetPos, DemandType.ArbitraryFeedForward, arbFeedFwd); //getFeedForward(0.15));
	...
	private double getFeedForward(double horizontalHoldOutput){
		...
		arbFeedFwd = gravityCompensation* horizontalHoldOutput;
		return arbFeedFwd;
	}
