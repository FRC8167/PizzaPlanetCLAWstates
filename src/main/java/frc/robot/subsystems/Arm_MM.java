...
public class Arm_MM extends SubsystemBase {
	...
	// private static double GEAR_RATIO = 12;
	// private static double MOTOR_COUNTS_PER_REV = 2048;
	private double ForwardSoftLimitThreshold = inches_To_Raw_Sensor_Counts(40);
	...
	public Arm_MM() {
		...
		// 12 to 1 Gearbox PID Constants
		_talon.config_kF(Constants.kSlotIdx, 0.00, Constants.kTimeoutMs);
		_talon.config_kP(Constants.kSlotIdx, 0.08, Constants.kTimeoutMs);
		_talon.config_kI(Constants.kSlotIdx, 0.0, Constants.kTimeoutMs);
		_talon.config_kD(Constants.kSlotIdx, 0.0, Constants.kTimeoutMs);

		/* Set acceleration and vcruise velocity - see documentation */
		
		// 20 to 1 Gearbox Magic Motion Parms
		// _talon.configMotionCruiseVelocity(18000, Constants.kTimeoutMs);
		// _talon.configMotionAcceleration(12500, Constants.kTimeoutMs);

		//******************** NEEDS UPDATED FROM SPREADSHET ************************/
		// 4 to 1 Gearbox Magic Motion Parms
		_talon.configMotionCruiseVelocity(20000, Constants.kTimeoutMs);
		_talon.configMotionAcceleration(48000, Constants.kTimeoutMs);
	...
	public void periodic() {
		...
		/*** Check that retract limit has not been exceeded ***/
		double vel = _talon.getSelectedSensorVelocity();
		double pos = _talon.getSelectedSensorPosition();
		...
		SmartDashboard.putNumber("Arm Motor Current", RobotContainer._pdm.getCurrent(15));
	...
	private double raw_Sensor_Counts_To_Inches(double counts){
		return (0.00011632*counts + 0.4371);
